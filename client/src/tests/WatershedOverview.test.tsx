...
describe("WatershedOverview", () => {
    ...
    describe("successful rendering", () => {
        ...
        it("renders back button", async () => {
            ...
            await waitFor(() => {
                const backButton = screen.getByRole("button", { name: /close watershed panel/i });
    ...
    describe("N/A fallbacks", () => {
        it("renders N/A when county is not available", async () => {
            mockUseMatch.mockReturnValue({ params: { webcloudRunId: "test-watershed-na" } });
    ...
    describe("different watershed selection", () => {
        it("renders correct watershed when ID changes", async () => {
            mockUseMatch.mockReturnValue({ params: { webcloudRunId: "test-watershed-456" } });
            mockFetchWatersheds.mockResolvedValue(mockWatershedData);
    ...
    describe("accessibility", () => {
        ...
        it("back button has proper aria-label", async () => {
            ...
            await waitFor(() => {
                const backButton = screen.getByRole("button", { name: /close watershed panel/i });
                expect(backButton).toHaveAttribute("aria-label", "Close watershed panel");
            });
        });
        ...
        it("action buttons have proper aria-labels", async () => {
            ...
            await waitFor(() => {
                const weppButton = screen.getByRole("button", { name: /View Calibrated WEPP Results/i });
                expect(weppButton).toHaveAttribute("aria-label", "View Calibrated WEPP Results");

                const rhessysButton = screen.getByRole("button", { name: /View Calibrated RHESSys Results/i });
                expect(rhessysButton).toHaveAttribute("aria-label", "View Calibrated RHESSys Results");
            });
        });
    });
});
